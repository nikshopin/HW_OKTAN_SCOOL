...
let cinderella:{name: string, age: number, shoeSize: number}[] = [
...
let prince: {name: string, age: number, findShoeSize: number} = new Prince('georgyi',25, 37.5);
...
let result: {name: string, age: number, shoeSize: number} | undefined = cinderella.find((value: {name: string, age: number, shoeSize: number} ): boolean=>
{
